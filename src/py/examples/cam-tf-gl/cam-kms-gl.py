...
from cam_gbm_egl import GbmEglSurface
...
class KMSState:
    def __init__(self, mybufs: list[MyBuf], queue_buf):
        ...
        out_width = self.mode.hdisplay
        out_height = self.mode.vdisplay

        # EGL & GL
        self.gbm_dev = gbm.GbmDevice(card.fd)
        self.egl_state = EglState(self.gbm_dev.handle)
        self.surface = GbmEglSurface(card, self.gbm_dev, self.egl_state,
        ...
        self.scene = GLScene()
        self.scene.setup(self.surface.egl_surface, mybufs)
        self.scene.set_viewport(out_width, out_height)
    ...
    def queue_next(self):
        ...
        # XXX give the buf back. It could still be used by the gpu, though...
        self.queue_buf(mybuf)
